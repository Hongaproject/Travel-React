...
function DetailBusan () {
    ...
    const [busanApi1, setBusanApi1] = useState([]);
    ...
    const textAPI = async () => {
        ...
        try {
            ...
            const res = await axios.get(`https://apis.data.go.kr/B551011/KorService1/detailCommon1?MobileOS=ETC&MobileApp=busan&_type=json&contentId=${idx}&contentTypeId=12&defaultYN=Y&firstImageYN=Y&addrinfoYN=Y&overviewYN=Y&serviceKey=${serviceKey}`);
            const res1 = await axios.get(`https://apis.data.go.kr/B551011/KorService1/detailIntro1?MobileOS=ETC&MobileApp=busan&_type=json&contentId=${idx}&contentTypeId=12&serviceKey=${serviceKey}`)
            console.log(res.data.response.body.items.item); 
            setBusanApi(res.data.response.body.items.item);
            setBusanApi1(res1.data.response.body.items.item);
        } catch (err) {
            setError(err); 
        }
        setLoading(false);
