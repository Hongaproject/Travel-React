...
function DaejeonHome () {
    ...
    const textAPI = async () => {
        try {
            ...
            const res = await axios.get(`https://apis.data.go.kr/B551011/KorService1/areaBasedList1?numOfRows=8&pageNo=1&MobileOS=ETC&MobileApp=daejeon&_type=json&contentTypeId=12&areaCode=3&serviceKey=${serviceKey}`);
                ...
                <div class="grid grid-cols-4 gap-4">
                    {
                        daejeonApi.slice(0,8).map((v) => (
                            <div className="p-10 bg-slate-200" key={v.contentid}>
                                <Link to={`/DetailDaejeon/${v.contentid}`} >
